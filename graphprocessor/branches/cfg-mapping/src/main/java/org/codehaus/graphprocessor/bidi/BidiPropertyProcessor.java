...
public class BidiPropertyProcessor implements PropertyProcessor
{
	...
	public void process(final PropertyContext pCtx, final Object source, final Object target)
	{
				...
				if (propertyConfig.isNode())
				{
					...
					if (nodeMapping != null)
					{
						...
						if (!isFiltered)
						{
							...
							final NodeProcessor trans = nodeCtx.getProcessingUnit().getProcessor();
		...
		{
			if (!((BidiPropertyProcessingUnit) pCtxImpl.getProcessingUnit()).getTargetProperty().isVirtualWrite())
			{
	...
	{
		final PropertyConfig pMap = ((BidiPropertyProcessingUnit) pCtx.getProcessingUnit()).getPropertyConfig();
	...
	private void writeValueToTarget(final PropertyContextImpl pCtx, final Object target, Object value)
	{
		PropertyConfig targetPropCfg = ((BidiPropertyProcessingUnit) pCtx.getProcessingUnit()).getTargetProperty();

		final Method writeMethod = targetPropCfg.getWriteMethod();
		...
		// invoke interceptor (if available)
		if (targetPropCfg.getWriteInterceptor() != null)
		{
			try
			{
				// when propertymapping is of type 'node' we have to do an additional compatibility check here because
				...
				//
				// }
				value = targetPropCfg.getWriteInterceptor().intercept(pCtx, value);
			}
			// any kind of exception gets caught, useful log output gets produced
			// property setter gets not invoked (method is left)
			catch (final Exception e)
			{
				final String name = targetPropCfg.getWriteInterceptor().getClass().getSimpleName();
				log.error(
